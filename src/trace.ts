...
export type TraceStepType = "plan" | "execute" | "tool";
...
export class TraceStore {
  ...
  printOne(index: number): void {
    ...
    if (!step) return;
    const title = `\n=== TRACE #${index + 1} ${step.type.toUpperCase()} ===`;
    console.log(title);
    if (step.model) {
      console.log(`model: ${step.model}`);
    }
    if (step.meta) {
      console.log(`meta: ${JSON.stringify(step.meta)}`);
    }
    console.log("--- prompt ---");
  ...
  summarizeTokenUsage(): TokenUsageSummary {
    ...
    for (const step of this.steps) {
      ...
      if (!byType[step.type]) {
        byType[step.type] = emptyBucket();
      }
      byType[step.type]!.prompt_tokens += p;
      byType[step.type]!.completion_tokens += c;
      byType[step.type]!.total_tokens += t;
      byType[step.type]!.cached_prompt_tokens += cached;
      byType[step.type]!.billed_prompt_tokens_est += billed;
      byType[step.type]!.reasoning_tokens += reasoning;
    }
